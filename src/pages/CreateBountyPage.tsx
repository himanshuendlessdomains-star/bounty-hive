      ...
      <div className="sticky top-0 bg-[var(--bg-primary)] z-40 p-4 border-b border-[var(--border)]">
        ...
        <div className="flex items-center gap-1">
          {CREATE_BOUNTY_STEPS.map((s) => (
            <div
              key={s.id}
              className={`flex-1 h-1 rounded-full ${s.id <= step ? 'bg-hive-500' : 'bg-[var(--border)]'}`}
            />
          ))}
        </div>
      ...
      <div className="p-4 space-y-4">
              ...
              {(['task', 'quiz', 'creative'] as const).map((t) => (
                ...
                >
                  <p className="text-white font-medium">
                    {t === 'task' ? '✅ Task' : t === 'quiz' ? '🧠 Quiz' : '🎨 Creative'}
                  </p>
        ...
        {step === 2 && (
          <>
            ...
            <textarea
              placeholder="Be specific about what proof you need..."
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              className="input-field min-h-[120px] resize-none"
              maxLength={500}
            />
            <p className="text-[var(--text-muted)] text-xs">{description.length}/500 characters</p>
          </>
        )}
                ...
                {poolAmount && winnerCount > 0 && (
                  <div className="border-t border-[var(--border)] pt-3">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className={minPayoutValid ? 'text-hive-500' : 'text-red-400'}>
                        ${perWinnerUsd.toFixed(4)}
                      </span>
                    </div>
                    {!minPayoutValid && (
                      <p className="text-red-400 text-xs mt-1">Minimum payout must be at least $0.01</p>
                    )}
                  </div>
                )}
              </div>
        ...
        {step === 4 && (
          <>
            ...
            <div className="space-y-3">
              <button
                onClick={() => setWinnerSelection('draw')}
              ...
              >
                ...
                <p className="text-[var(--text-muted)] text-xs">
                  Winners are randomly selected from approved submissions
                </p>
              </button>
              <button
                onClick={() => setWinnerSelection('manual')}
                className={`w-full text-left p-4 rounded-xl border transition-colors ${
              ...
              >
                ...
                <p className="text-[var(--text-muted)] text-xs">
                  You choose the winners from approved submissions
                </p>
              </button>
            </div>
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <p className="text-[var(--text-muted)] text-xs">
                  You review and approve each submission
                </p>
              </button>
              <button
                onClick={() => setVerification('auto')}
                className={`w-full text-left p-4 rounded-xl border transition-colors ${
                  verification === 'auto'
                    ? 'border-hive-500 bg-hive-500/10'
                    : 'border-[var(--border)] hover:border-[var(--border-focus)]'
              ...
              >
                ...
                <p className="text-[var(--text-muted)] text-xs">
                  Submissions are automatically approved after 24h if you don't review
                </p>
              </button>
            </div>
          </>
            ...
            <div className="card space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">
                  {type === 'task' ? '✅ Task' : type === 'quiz' ? '🧠 Quiz' : '🎨 Creative'}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-[var(--text-secondary)]">Pool</span>
                <span className="text-hive-500 font-bold">{poolAmount} TON</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[var(--text-secondary)]">Winners</span>
                <span className="text-white">{winnerCount}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[var(--text-secondary)]">Per winner</span>
                <span className="text-white">{perWinner} TON</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[var(--text-secondary)]">Selection</span>
                <span className="text-white">
                  {winnerSelection === 'draw' ? '🎲 Random' : '✋ Manual'}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-[var(--text-secondary)]">Verification</span>
                <span className="text-white">
                  {verification === 'manual' ? '👁 Manual' : '⚡ Auto'}
                </span>
              </div>
              <div className="flex justify-between">
